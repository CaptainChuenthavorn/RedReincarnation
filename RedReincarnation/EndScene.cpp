...
EndScene::EndScene(float width, float height)
{
	...
	Endmenu[0].setPosition(sf::Vector2f(width / 2 - 200, height / (MAX_ITEMS + 1) * 2));//Remenu[0].setPosition(sf::Vector2f(width/2-200,height/(MAX_ITEMS+1)*1 - 100));
	...
	Endmenu[1].setPosition(sf::Vector2f(width / 2 - 200, height / (MAX_ITEMS + 1) * 2));//	Remenu[1].setPosition(sf::Vector2f(width / 2-200, height / (MAX_ITEMS + 1) * 2 - 200));
}
